...
public class DialogCreateGraph extends JDialog {
	...
	private boolean hColChartType, vColChartType;
	...
	private void chartTypeMinMaxAttr(){
		...
		if (!validateMinMaxValue()) {
			...
			if (chartChoice == 1) {
				...
				if (smallestPeekX - bufferX < 0){
					bufferX = 1;
				}
				if (smallestPeekY - bufferY < 0){
					bufferY = 1;
				}
				
				axisXMinValue.setText(smallestPeekX - bufferX + "");
				axisXMaxValue.setText(biggestPeekX + bufferX + "");
				axisXDiv.setValue(((int) attrNodes.getValue()) + 1);
			...
			if (chartChoice == 2) {
				...
				axisYDiv.setValue(((int) attrNodes.getValue()) + 1);
			}
		}
	}
	...
	public void divValues(){
		if (ImportGraphDataListener.isImported()){
			...
			if (!hColChartType && !vColChartType){
				axisXDiv.setValue(divXValue);
				axisYDiv.setValue(divYValue);
			}
			else if (hColChartType){
				axisYDiv.setValue(divYValue);
			}
			else if (vColChartType){
				axisXDiv.setValue(divXValue);
			}
		}
	}
